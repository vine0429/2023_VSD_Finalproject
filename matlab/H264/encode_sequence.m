...
% QCIF frames are stored as JPEG in the directory 
listing = dir(dirname);
filename = [dirname '\' listing(3,1).name]; % images start at 3
...
%         Seq(:,:,2) = uvc_read_frame(fid,k+Frame_start);
        filename = [dirname '\' listing(3+k,1).name]; % new reading mechanism
        Seq(:,:,2) = double(imread(filename));
